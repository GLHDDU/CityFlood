...
u8  USART3_RX_ABUF[USART3_MAX_RECV_LEN]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节 
u8  USART3_RX_BBUF[USART3_MAX_RECV_LEN]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节 
u8  USART3_RX_CBUF[USART3_MAX_RECV_LEN]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节 
...
void USART3_RX_Data()
{
	...
	if(USART3_RX_STA&0x8000)
		{	
			...
			{
				if(sensor_souce==1)  //来自入口传感器
				USART3_RX_ABUF[i]=USART3_RX_BUF[i];
				if(sensor_souce==2)  //来自中段传感器
				USART3_RX_BBUF[i]=USART3_RX_BUF[i];
				if(sensor_souce==3)  //来自出口传感器

...
{	
	...
	Sensor_Data->WaterLevel1,Sensor_Data->Rainfall_index1,Sensor_Data->Current_temperature1,Sensor_Data->Current_humidity1,Sensor_Data->water_quality1);  //构建回复湿度温度数据
...
sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":\
	...
	Sensor_Data->WaterLevel2,Sensor_Data->Rainfall_index2,Sensor_Data->Current_temperature2,Sensor_Data->Current_humidity2,Sensor_Data->water_quality2);  //构建回复湿度温度数据
		MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));   //添加数据，发布给服务器

	sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":\
	{\"WaterLevel3\":%2d,\"Rainfall_index3\":%2d,\"Current_temperature3\":%2d,\"Current_humidity3\":%2d,\"water_quality3\":%2d,},\"version\":\"1.0.0\"}",\
	Sensor_Data->WaterLevel3,Sensor_Data->Rainfall_index3,Sensor_Data->Current_temperature3,Sensor_Data->Current_humidity3,Sensor_Data->water_quality3);  //构建回复湿度温度数据
		MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));   //添加数据，发布给服务器
